...
### 2026-09-05 WorldMap 输入框与情绪入口更新

- 输入框的提示图/无字图必须是 Scene 中预先保存的两套 Image；运行时只切换显隐，不动态替换 Sprite 或创建 UI。
- 情绪输入入口以真实的 `天使标牌`、`恶魔标牌` 为准，旧的 EmotionEntry 占位对象不得继续作为点击入口；右上角 `Btn_EmotionInput` 应停用。
- 图鉴卡片与详情页不显示土壤，土壤只属于每周培育面板；作者化脚本和运行时脚本都要保持这一边界。
